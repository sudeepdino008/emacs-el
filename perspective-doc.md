...
list is easier Ñ in which case, keeping the list well-pruned for relevant
...
windows Ñ all without losing any of the context for your work on bug Y or
feature X.
...
- `s` Ñ `persp-switch`: Query a perspective to switch to, or create
- `k` Ñ `persp-remove-buffer`: Query a buffer to remove from current perspective
- `c` Ñ `persp-kill` : Query a perspective to kill
- `r` Ñ `persp-rename`: Rename current perspective
- `a` Ñ `persp-add-buffer`: Query an open buffer to add to current perspective
- `A` Ñ `persp-set-buffer`: Add buffer to current perspective and remove it from all others
...
- `i` Ñ `persp-import`: Import a given perspective from another frame.
- `n`, `<right>` Ñ `persp-next`: Switch to next perspective
- `p`, `<left>` Ñ `persp-prev`: Switch to previous perspective
- `C-s` Ñ `persp-state-save`: Save all perspectives in all frames to a file
- `C-l` Ñ `persp-state-load`: Load all perspectives from a file
